...
add constraint fk_forn_prod foreign key (fornecedor) references fornecedores (id_fornecedor);
...
add constraint fk_prod_forn foreign key (id_produto) references produtos (id_produto),
...
add constraint fk_quant_pef foreign key (quant_disponivel) references produtos (quant_disponivel),
